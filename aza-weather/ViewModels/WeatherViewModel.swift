...
class WeatherViewModel: ObservableObject {
    private var azaWeatherService: WeatherService!
    ...
    init() {
        self.azaWeatherService = WeatherService()
        weatherDate = self.weatherResponse.dt
    }
    
    /// Format the date properly (e.g. Monday, Jan 15, 2024)
    ...
    /// Concatenate the city and country code (City of Johannesburg, ZA).
...
//        }
    ...
    public func getWeatherByZipCode(by zip: String, country_code: String) {
        self.azaWeatherService.getWeatherByZipCode(zip: zip, country_code: country_code) { weather in
    ...
    private func fetchWeather(by city: String, byCoordinates: Bool, lat: Double, long: Double) {
        ...
        self.azaWeatherService.getWeather(city: city, byCoordinates: byCoordinates, lat: lat, long: long) { weather  in
